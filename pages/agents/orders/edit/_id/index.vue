<template>
  ...
  <v-layout>
    <v-tabs>
      <v-tab v-for='(item, index) in intabs' :key='index'>
        <span>{{item.title}}</span>
      </v-tab>
      <v-tab-item v-for='n in intabs.length' :key='n'>
        <v-content v-if='n === 1'>
        <v-layout  >
            <v-flex xs12 sm5 md3>
            </v-flex>
        ...
        <v-content v-if='n === 2'>
          <v-layout>
              <v-flex xs12 sm5 md3>
                ...
                <v-btn flat slot='activator' ><v-icon large>shopping_cart</v-icon> Go cart</v-btn>
                <v-card>
                <v-card-title
