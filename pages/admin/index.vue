  ...
  <v-container>
            ...
            <v-flex>
              <v-toolbar dark>
                ...
                <v-toolbar-items>
                  <v-btn 
                    flat 
                    to="/admin/agent/order"><span>All orders</span> </v-btn>
                </v-toolbar-items>
              </v-toolbar>
            </v-flex>
          </v-layout>
          <v-layout>
            <v-flex>
              <v-card>
                <span 
                  v-if="orderNum < 1" 
                  class="display-1">Your agents did not order yet</span>
                <order-dash 
                  v-else 
                  :items="listOrder" />
              </v-card>
        ...
        <v-container>
          <v-card>
            <v-tabs v-model="primary">
              <v-tab 
                v-for="(item, index) in primaryhead" 
                :key="index" 
                ripple>
                {{ item.title }}
              </v-tab>
              <v-tab-item 
                v-for="n in primaryhead.length" 
                :key="n" >
                <v-layout v-if="n===1">
                  <v-flex>
                    <span 
                      v-if="conflen < 1" 
                      class="subheading">No confirmations on orders</span>
                    <v-list v-else>
                      <v-btn 
                        flat 
                        to="/admin/agent/quotation"><span class="grey--text"> View all list</span></v-btn>
                      <v-list-group 
                        v-for="(conf, index) in confall" 
                        :key="index">
                ...
                <v-layout v-if="n===2">
                  <v-flex>
                    <v-list>
                      <v-subheader>Full Payments <v-btn 
                        to="/admin/agent/payment/full" 
                        flat><span class="grey--text"> View All</span></v-btn></v-subheader>
                      <payfull :items="payfulldat"/>
                      <v-subheader>Half Payments <v-btn 
                        to="/admin/agent/payment/half" 
                        flat><span class="grey--text"> View All</span></v-btn></v-subheader>
    ...
    <v-layout 
      ...
      <v-flex 
        ...
        <span 
          v-if="agentNum <1" 
          class="display-1">You have no Employees / Agents</span>
        <v-select 
          v-else 
          v-model="lAgent" 
          :items="listAgents" 
            ...
            <v-list-group>
              ...
              <v-list-tile>
                <v-list-tile-action>
                  <v-btn 
                    flat 
                    @click="detailclient(lAgent)"> <span class="blue--text">View all client</span></v-btn>
                </v-list-tile-action>
              </v-list-tile>
              <v-list-tile>
                <v-list-tile-action>
                  <v-btn 
                    flat 
                    @click="detailorder(lAgent)"> <span class="orange--text">View all order</span></v-btn>
                </v-list-tile-action>
              </v-list-tile>
              <v-list-tile>
                <v-list-tile-action>
                  <v-btn 
                    flat 
                    @click="detailfullpay(lAgent)"> <span class="green--text">View all full payments</span></v-btn>
                </v-list-tile-action>
              </v-list-tile>
              <v-list-tile>
                <v-list-tile-action>
                  <v-btn 
                    flat 
                    @click="detailhalfpay(lAgent)"> <span class="green--text">View all half payments</span></v-btn>
                </v-list-tile-action>
              </v-list-tile>
            </v-list-group>
          </v-list>
