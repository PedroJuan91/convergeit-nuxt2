        ...
        <v-layout
          ...
          <v-card-title>
            ...
            <v-btn
              flat
              icon
              @click="removeProd(index)"><v-icon>close</v-icon></v-btn>
          </v-card-title>
          <v-spacer />
        ...
        <v-list-tile>
          ...
          <v-list-tile-action>
            <v-btn
              flat
              icon
              @click="incrementVal(item.id)"><v-icon>add</v-icon></v-btn>
          </v-list-tile-action>
          <v-list-tile-action>
            <v-btn
              ...
              flat
              icon
              @click="decrementVal(item.id)" ><v-icon>remove</v-icon></v-btn>
          </v-list-tile-action>
        </v-list-tile>
        <v-list-group >
          ...
          <v-list-tile
            v-for="(items, index) in item.proddesc"
            :key="index">
...
import {mapGetters, mapActions} from 'vuex'
  export default {
    ...
    computed: {
      ...mapGetters('order',{
        items: 'viewCheckoutList',
        increment: 'incrementValue'
      })
    },
    methods: {
      onLoadAgent(id) {
        return console.log('you click id ' + id)
      },
      incrementVal(id) {
        this.$store.dispatch('order/invermentValue', id)
        .catch(e => {
          this.text = e.message
          this.snackbar = true
        })
      ...
      decrementVal(id) {
        this.$store.dispatch('order/decrementValue', id)
        .catch(e => {
          this.text = e.message
          this.snackbar = true
        })
      ...
      removeProd(index) {
        this.$store.dispatch('order/removeProd', index)
      }
    }
