<template>
  <v-flex>
    <v-flex>
      <v-list >
        <v-subheader >Paid in Full Payment <v-btn
          to="/agents/projects/payments/full"
          flat
          ripple> <span class="grey--text">View all full payments</span></v-btn></v-subheader>
    ...
    <v-flex>
      <v-list>
        <v-subheader >Paid in Half Payment <v-btn
          flat
          ripple
          to="/agents/projects/payments/half"> <span class="grey--text">View all half payments</span></v-btn></v-subheader>
        <v-list-tile v-if="lenhalfpay < 1 ">
          <v-list-tile-title ><span class="body-1">No client paid in half</span> </v-list-tile-title>
        </v-list-tile>
        <v-list-group
          v-for="(items, index) in halfpayments"
          v-else
          :key="index">
          <v-list-tile slot="activator">
